...
type LxdCLogger struct {
	Config *specs.LxdComposeConfig
	Logger *zap.Logger
	Aurora aurora.Aurora
}

var defaultLogger *LxdCLogger = nil

func NewLxdCLogger(config *specs.LxdComposeConfig) *LxdCLogger {
	return &LxdCLogger{
		Logger: nil,
...
func (l *LxdCLogger) GetAurora() aurora.Aurora {
	return l.Aurora
}

func (l *LxdCLogger) SetAsDefault() {
	defaultLogger = l
}

func GetDefaultLogger() *LxdCLogger {
	return defaultLogger
}

func (l *LxdCLogger) InitLogger2File() error {
	var err error
...
func (l *LxdCLogger) log2File(level, msg string) {
	switch level {
	case "error":
		l.Logger.Error(msg)
	case "warning":
