...
import (
	...
	incus_units "github.com/canonical/lxd/shared/units"
...
func (e *IncusExecutor) RecursiveMkdir(nameContainer string, dir string, mode *os.FileMode, uid int64, gid int64) error {
	...
	for ; i >= 1; i-- {
		...
		_, resp, err := e.Client.GetInstanceFile(nameContainer, cur)
		if err != nil {
			continue
		}
	...
	for ; i <= len(parts); i++ {
		...
		err := e.Client.CreateInstanceFile(nameContainer, cur, args)
		if err != nil {
			return err
		}
	}
...
func (e *IncusExecutor) RecursivePushFile(nameContainer, source, target string) error {
	...
	sendFileInstance := func(p string, fInfo os.FileInfo, err error) error {
		...
		if args.Type != "directory" {
			...
			args.Content = helpers.NewReadSeeker(&ioprogress.ProgressReader{
								...
								logger.Aurora.Italic(
									logger.Aurora.BrightMagenta(
										fmt.Sprintf("%d%% (%s/s)", percent,
											incus_units.GetByteSizeString(speed, 2)))))
						}
		...
		err = e.Client.CreateInstanceFile(nameContainer, targetPath, args)
		if err != nil {
			if args.Type != "directory" {
				progress.Done("")
			}
...
func (l *IncusExecutor) RecursivePullFile(nameContainer string, destPath string, localPath string, localAsTarget bool) error {
	...
	var resp *incus.InstanceFileResponse
	buf, resp, err = l.Client.GetInstanceFile(nameContainer, destPath)
	if err != nil {
		return err
	}
			...
			Tracker: &ioprogress.ProgressTracker{
				Handler: func(bytesReceived int64, speed int64) {
					...
					l.Emitter.DebugLog(false, fmt.Sprintf("%s (%s/s)\n",
						incus_units.GetByteSizeString(bytesReceived, 2),
						incus_units.GetByteSizeString(speed, 2)))
...
func (e *IncusExecutor) recursiveListFile(nameContainer string, targetPath string, list *list.List) error {
	var ftype string
	var entries []string
	var buf io.ReadCloser
	var err error
	...
	var resp *incus.InstanceFileResponse
	buf, resp, err = e.Client.GetInstanceFile(nameContainer, targetPath)
	if err != nil {
		return err
	}
	ftype = resp.Type
	...
	if buf != nil {
		// Needed to avoid: dial unix /var/lib/incus.unix.socket: socket: too many open files
		buf.Close()
	}
...
func (e *IncusExecutor) DeleteContainerDir(name, dir string) error {
	...
	for f := list.Front(); f != nil; f = f.Next() {
		...
		err = e.Client.DeleteInstanceFile(name, f.Value.(string))
		if err != nil {
